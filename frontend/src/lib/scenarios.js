...
export const SCENARIO_REGISTRY = {
  ...
  dropping_or_throwing_precursor: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE CONTROLLED LOWERING →',
  ...
  carton_drop: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE CONTROLLED DESCENT →',
  ...
  dragging_precursor: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE WHEELED APPARATUS →',
  ...
  rolling_precursor: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE UPRIGHT TRANSPORT →',
  ...
  straps_as_handles: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE BASE LIFT TECHNIQUE →',
  ...
  stepping_on_carton: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE SAFETY STEP STOOL →',
  ...
  stepping_on_carton_precursor: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE SAFETY ACCESS EQUIPMENT →',
  ...
  entity_in_dock_edge_zone: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE DEPLOYED DOCK BARRIER →',
  ...
  entity_in_wet_floor_zone: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE DRY PATH REROUTE →',
  ...
  unplanned_loading_sequence: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE MANIFEST SEQUENCE →',
  ...
  solo_heavy_handling: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE TEAM LIFT ASSIST →',
  ...
  wrong_equipment_usage: {
    ...
    whatIfEligible: true,
    whatIfCta: 'SIMULATE CERTIFIED PALLET TRUCK →',
...
export const RISK_BAND_STYLES = {
  Critical: {
    ...
    subtle: 'border-danger/40 bg-danger/10 text-danger',
    dot: 'bg-danger',
  },
  High: {
    label: 'high risk',
    badge: 'bg-signal text-ink',
    subtle: 'border-signal/40 bg-signal/10 text-[#8a5f00]',
    dot: 'bg-signal',
  },
  Medium: {
    label: 'medium risk',
    badge: 'bg-steel text-paper',
    subtle: 'border-steel/40 bg-steel/10 text-steel',
    dot: 'bg-steel',
  },
  Low: {
    label: 'low risk',
    badge: 'bg-ink-faint text-paper',
    subtle: 'border-line-strong bg-paper text-ink-soft',
    dot: 'bg-line-strong',
  },
}
...
export const STATUS_STYLES = {
  ...
  unsupported: {
    ...
    style: 'border-line bg-paper text-ink-faint',
    dot: 'bg-line',
  },
}
...
export function getSimplifiedInterventions(scenarioKey, current, alternatives = []) {
    ...
    return [
      {
        ...
        badgeStyle: 'bg-ok text-paper',
        title: 'Invert Tier Order: Heavy Base Deck',
        predictedRisk: 'low risk',
        predictedRiskStyle: 'text-ok bg-ok/10 border-ok/40',
        score: scoreA,
      ...
      },
      {
        ...
        badgeStyle: 'bg-steel text-paper',
        title: 'Stage Upper Cargo to Adjacent Pallet',
        predictedRisk: 'low risk',
        predictedRiskStyle: 'text-steel bg-steel/10 border-steel/40',
        score: scoreB,
      ...
      },
      {
        ...
        badge: 'status quo',
        badgeStyle: 'bg-danger text-paper',
        title: 'Maintain Current Inverted Stack',
        predictedRisk: 'high risk',
        predictedRiskStyle: 'text-danger bg-danger/10 border-danger/40',
        score: currentScore,
        scoreDelta: 0,
  ...
  return [
    ...
    },
    {
      ...
      score: currentScore,
      scoreDelta: 0,
      whySafer: 'No intervention. Cantilever overhang remains unmitigated with high risk of tipping during pallet movement.',
      cta: 'Simulate Failure Risk',
      candidate: null,
    },
  ]
...
export function getPhysicalStackComparison(scenarioKey, current, proposed) {
  ...
  if (isHeavyOnLight) {
    return {
      current: {
        top: { label: 'Heavy Cargo Package', status: 'Misplaced Upper Tier', color: 'border-danger/40 bg-danger/10 text-danger' },
        arrow: '↓ High compressive crushing pressure',
        bottom: { label: 'Lighter Base Carton', status: 'Crush Hazard Under Load', color: 'border-signal/40 bg-signal/10 text-[#8a5f00]' },
        riskBadge: 'HIGH CRUSH RISK',
        riskBadgeStyle: 'bg-danger text-paper',
      },
      proposed: {
        top: { label: 'Lighter Cargo Package', status: 'Safe Upper Tier', color: 'border-ok/40 bg-ok/10 text-ok' },
        arrow: '↓ Stable downward mass distribution',
        bottom: { label: 'Heavy Base Foundation', status: 'Solid Base Support Deck', color: 'border-steel/40 bg-steel/10 text-steel' },
        riskBadge: 'BALANCED FOUNDATION',
        riskBadgeStyle: 'bg-ok text-paper',
      },
      claims: [
  ...
  if (isOrientation) {
    return {
      current: {
        top: { label: 'Package Horizontal / Inverted', status: 'Violates This-Side-Up Rule', color: 'border-danger/40 bg-danger/10 text-danger' },
        arrow: '↓ Lateral seam stress & internal shifting',
        bottom: { label: 'Supporting Deck', status: 'Uneven Surface Contact', color: 'border-line bg-paper text-ink-soft' },
        riskBadge: 'RULE VIOLATION',
        riskBadgeStyle: 'bg-danger text-paper',
      },
      proposed: {
        top: { label: 'Package Upright ("This Side Up")', status: 'Conforming Label Orientation', color: 'border-ok/40 bg-ok/10 text-ok' },
        arrow: '↓ Vertical column compression aligned',
        bottom: { label: 'Supporting Deck', status: 'Even Footprint Distribution', color: 'border-steel/40 bg-steel/10 text-steel' },
  ...
  return {
    current: {
      top: { label: 'Upper Carton', status: `Cantilever Overhang (${Math.round(overhangPct)}% off base)`, color: 'border-danger/40 bg-danger/10 text-danger' },
      arrow: '↓ Eccentric tipping load',
      bottom: { label: 'Weak Deck Support', status: `Only ${Math.round(supportPct)}% Base Contact`, color: 'border-signal/40 bg-signal/10 text-[#8a5f00]' },
      riskBadge: 'HIGH TIPPING RISK',
      riskBadgeStyle: 'bg-danger text-paper',
    },
    proposed: {
      top: { label: 'Upper Carton', status: 'Centered Support Footprint', color: 'border-ok/40 bg-ok/10 text-ok' },
      arrow: '↓ Symmetrical downward load',
      bottom: { label: 'Full Base Support', status: `${Math.round(propSupportPct)}% Foundation Coverage`, color: 'border-steel/40 bg-steel/10 text-steel' },
