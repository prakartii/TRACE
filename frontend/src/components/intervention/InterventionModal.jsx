...
export default function InterventionModal({ alert, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
      <div className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-line bg-paper shadow-2xl">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-line bg-paper-subtle p-5">
          <div className="flex items-start gap-3">
            <div
              className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${
                isCritical ? 'bg-danger text-white' : isHigh ? 'bg-signal text-ink' : 'bg-steel text-white'
              }`}
            >
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-2 text-ink-faint hover:bg-paper hover:text-ink"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="rounded-xl border border-ink/20 bg-ink p-4 text-paper shadow-xs">
            <span className="text-[11px] font-bold uppercase tracking-wider text-paper/70">
              Immediate Operational Action
            </span>
            <p className="mt-1 text-base font-bold text-paper">
            ...
            <div>
              ...
              <ol className="mt-2 space-y-2">
                {alert.steps.map((step, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-3 rounded-lg border border-line bg-paper-subtle p-3 text-sm text-ink"
                  >
          ...
          {/* 3. WHY */}
          <div className="rounded-xl border border-line bg-paper-subtle p-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-ink-soft">
          ...
          <div className="border-t border-line pt-3">
            <button
              ...
              className="flex w-full items-center justify-between py-1 text-xs font-semibold text-ink-soft hover:text-ink"
            >
            ...
            {showTechnical && (
              <div className="mt-3 space-y-2 rounded-lg border border-line bg-paper-subtle p-4 text-xs text-ink-soft">
          ...
          {showDismissInput && (
            <div className="rounded-xl border border-danger/30 bg-danger/5 p-4">
              <h4 className="text-xs font-bold text-danger">Mark Hazard as False Positive</h4>
              ...
              <textarea
                ...
                className="mt-2 w-full rounded-lg border border-line bg-paper p-2 text-xs text-ink outline-none focus:border-danger"
              />
              <div className="mt-2 flex items-center justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowDismissInput(false)}
                  className="rounded-lg border border-line bg-paper px-3 py-1.5 text-xs text-ink"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="rounded-lg bg-danger px-3 py-1.5 text-xs font-bold text-white hover:bg-danger/90 disabled:opacity-50"
                >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line bg-paper-subtle p-4">
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleNavigateReplay}
              className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-paper px-3 py-2 text-xs font-medium text-ink hover:bg-paper-subtle active:scale-95"
            >
            ...
            {!showDismissInput && (
              <button
                ...
                className="rounded-lg px-2.5 py-2 text-xs text-ink-faint hover:text-danger"
              >
                Mark False Positive
              </button>
            )}
          ...
          <div className="flex items-center gap-2">
            {alert.state === 'NEW' && (
              <button
                ...
                disabled={loading}
                className="inline-flex items-center gap-1.5 rounded-lg bg-ink px-4 py-2 text-xs font-bold text-paper shadow-sm hover:bg-ink/90 active:scale-95 disabled:opacity-50"
              >
            ...
            {alert.state === 'ACKNOWLEDGED' && (
              <button
                ...
                disabled={loading}
                className="inline-flex items-center gap-1.5 rounded-lg bg-signal px-4 py-2 text-xs font-bold text-ink shadow-sm hover:bg-signal/90 active:scale-95 disabled:opacity-50"
              >
