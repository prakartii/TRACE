...
from backend.contracts.models import EntityClass, PerceptionFrameResult
    ...
    for frame_result in frame_results:
        for entity in frame_result.entities:
            ...
            samples_by_id.setdefault(entity.id, []).append(
                TrackSample(timestamp=frame_result.timestamp, position=position, confidence=entity.confidence)
            )
            class_by_id[entity.id] = entity.entity_class

    return {
        entity_id: TrackHistory(
            entity_id=entity_id,
